...
// Reference your select elements
...
// Call this on load or when user opens the menu
populateDeviceList();

// Handle camera change
videoSelect.addEventListener('change', (e) => {
  pexrtc.video_source = e.target.value;
  pexrtc.renegotiate(false); // Apply the change
});

// Handle microphone change
audioSelect.addEventListener('change', (e) => {
  pexrtc.audio_source = e.target.value;
  pexrtc.renegotiate(false);
});

// Optional: handle speaker (audio output) change
audioOutputSelect.addEventListener('change', (e) => {
  const videoElement = document.querySelector('video'); // Or your actual video element
  if (typeof videoElement.sinkId !== 'undefined') {
    videoElement.setSinkId(e.target.value).catch(err => {
