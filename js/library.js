...
unregister: function () {
  ...
  const conferenceAlias = 'drdemo'; // <-- Replace with actual conference alias
  const nodeAddress = 'cklab-edgenc01.ck-collab-engtest.com';
  ...
  async function getToken() {
    try {
      // Fetch token by calling the request_token endpoint
      const response = await fetch(`https://${nodeAddress}/api/client/v2/conferences/${conferenceAlias}/request_token`);
      ...
      const data = await response.json();
      return data.token; // Assumes the response is { token: 'your_token' }
    } catch (err) {
  ...
  async function setupEventSource() {
    ...
    if (originalIp === null) {
      originalIp = ip;
    } else if (ip !== originalIp) {
      console.log('IP has changed from', originalIp, 'to', ip);
      originalIp = ip;
      restartEventSource();
      return;
    }
    ...
    if (!currentToken) {
      console.error('No token found, unable to set up EventSource');
      return;
    }
  
    if (!eventSource || eventSource.readyState === EventSource.CLOSED) {
      console.log('Creating new EventSource connection...');
      const eventUrl = `https://${nodeAddress}/api/client/v2/conferences/${conferenceAlias}/events?token=${currentToken}`;
      ...
      eventSource.onerror = (err) => {
        ...
        if (err.status === 401) {
          // Handle token expiration (e.g., fetch new token and restart EventSource)
          console.log('Token expired. Fetching new token...');
          setupEventSource();  // Re-run to fetch a new token
        }
      };
    }
  }
  ...
  function restartEventSource() {
    ...
    setupEventSource(); // Re-establish connection
  }
  
  // Check every 60 seconds
  setInterval(setupEventSource, 60000);
